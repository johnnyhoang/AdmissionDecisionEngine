...
- `grade10_schools`: Danh sách trường THPT công lập tại TP.HCM. Ngoài thông tin cơ bản (tên, mã, quận, địa chỉ, website) còn có: `description` (giới thiệu), `comments` (đánh giá chung), `activities` (hoạt động & phong trào — text dài, nhập liệu dần), tọa độ `latitude`/`longitude` phục vụ tính khoảng cách, `mapUrl`, `isVerified`.
