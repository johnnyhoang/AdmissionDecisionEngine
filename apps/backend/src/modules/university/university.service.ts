...
export class UniversityService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    ...
    if (methodCount === 0) {
      console.log(
        'No admission methods found. Seeding base admission methods...',
      );
      await this.seedAdmissionMethodsIfMissing();
      console.log('Base admission methods seeded successfully!');
    } else {
      console.log(
        `Database ready. Found ${methodCount} admission methods, ${await this.universityRepository.count()} universities.`,
      );
    }
  }
    ...
    const methods = [
      {
        ...
        description:
          'Xét tuyển dựa trên kết quả kỳ thi tốt nghiệp trung học phổ thông quốc gia.',
      },
      {
        code: 'HOCBA',
      ...
      },
      {
        ...
        description:
          'Xét tuyển dựa trên kết quả kỳ thi Đánh giá năng lực của Đại học Quốc gia Hà Nội (HSA).',
      },
      {
        code: 'COMBINED',
    ...
    for (const prog of programs) {
      for (const rule of prog.admissionRules) {
        if (rule.admissionMethod.code === 'THPT') {
          // Focus on THPT for standard benchmarking
  ...
  async seedInitialData() {
    ...
    const majorsData = [
      {
        ...
        sector: 'IT',
        description:
          'Nghiên cứu về cơ sở lý thuyết của thông tin và tính toán.',
      },
      {
        code: '7480103',
        ...
        sector: 'IT',
        description:
          'Nghiên cứu về quy trình thiết kế, phát triển và bảo trì phần mềm.',
      },
      {
        code: '7480201',
        ...
        sector: 'IT',
        description:
          'Sử dụng hệ thống máy tính và viễn thông để lưu trữ, truyền dữ liệu.',
      },
      {
        code: '7480107',
        ...
        sector: 'IT',
        description:
          'Nghiên cứu mô phỏng trí thông minh của con người cho máy móc.',
      },
      {
        code: '7340101',
        ...
        sector: 'Economics',
        description:
          'Quản lý hoạt động kinh doanh và đưa ra quyết định chiến lược.',
      },
      {
        code: '7310101',
        nameVi: 'Kinh tế học',
    ...
    for (const u of universitiesData) {
      const university = await this.universityRepository.save(
        this.universityRepository.create(u),
      );

      // Create a Campus
      const campus = await this.campusRepository.save(
        this.campusRepository.create({
      ...
      if (u.code === 'QSC') {
        ...
        const csProgram = await this.programRepository.save(
        ...
        const seProgram = await this.programRepository.save(
          this.programRepository.create({
            universityId: university.id,
        ...
        const csThptRule = await this.ruleRepository.save(
        ...
        // Rule 2: DGNL HCM
        const csDgnlRule = await this.ruleRepository.save(
          this.ruleRepository.create({
            programId: csProgram.id,
        ...
        const seThptRule = await this.ruleRepository.save(
          this.ruleRepository.create({
