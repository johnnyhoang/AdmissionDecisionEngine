...
import type { G10SchoolItem, G10SchoolDetail } from "../../../services/api";
...
export default function MergeSchoolModal({
  isOpen,
  onClose,
  school1,
  school2,
  onMerge,
}: MergeSchoolModalProps) {
  const [mergedData, setMergedData] = useState<any>({});
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [school1Detail, setSchool1Detail] = useState<G10SchoolDetail | null>(
    null,
  );
  const [school2Detail, setSchool2Detail] = useState<G10SchoolDetail | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      if (isOpen && school1 && school2) {
        setIsLoadingDetails(true);
        try {
          const [detail1, detail2] = await Promise.all([
            fetchG10SchoolDetail(school1.id),
            fetchG10SchoolDetail(school2.id),
          ]);
          if (cancelled) return;
          setSchool1Detail(detail1);
          setSchool2Detail(detail2);
          const initialData: any = {};
          FIELDS.forEach((f) => {
            initialData[f.key] =
              (detail1 as any)[f.key] || (school1 as any)[f.key] || "";
          });
          ...
          if (!cancelled) {
            ...
            setMergedData(initialData);
            setSchool1Detail(null);
            setSchool2Detail(null);
          }
        } finally {
  ...
  }, [isOpen, school1, school2]);

  if (!isOpen || !school1 || !school2) return null;

  const leftSchool = school1Detail || school1;
  const rightSchool = school2Detail || school2;
      ...
      <div className="relative bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-6xl max-h-[90vh] flex flex-col shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-slate-800">
          <h2 className="text-xl font-bold text-slate-100 flex items-center gap-2">
            Merge 2 Trường
            <span className="text-sm font-normal px-2 py-1 bg-indigo-500/20 text-indigo-300 rounded">
              Trường 1 (Chính) & Trường 2 (Phụ sẽ bị xoá)
            </span>
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="grid grid-cols-12 gap-4 text-sm font-semibold text-slate-400 border-b border-slate-800 pb-2 mb-4">
            ...
            <div className="col-span-3 text-indigo-300">
              Trường chính (Giữ lại)
            </div>
            <div className="col-span-3 text-purple-300">
              Trường phụ (Sẽ gộp & xoá)
            </div>
            <div className="col-span-4 text-emerald-300">
              Dữ liệu sau khi gộp (Sẽ lưu)
            </div>
          </div>

          <div className="flex flex-col gap-4">
            {FIELDS.map((field) => {
              const val1 = (leftSchool as any)[field.key] || "";
              const val2 = (rightSchool as any)[field.key] || "";
              const isDiff = val1 !== val2;

              return (
                <div
                  key={field.key}
                  className={`grid grid-cols-12 gap-4 items-start p-3 rounded-xl border ${isDiff ? "bg-amber-950/20 border-amber-900/30" : "border-transparent"}`}
                >
                  <div className="col-span-2 font-medium text-slate-300 flex flex-col gap-1">
                    {field.label}
                    {isDiff && (
                      <span className="text-[10px] uppercase bg-amber-500/20 text-amber-400 px-1.5 py-0.5 rounded w-max">
                        Khác biệt
                      </span>
                    )}
                  </div>
                  ...
                  >
                    <div
                      className={`text-sm line-clamp-4 whitespace-pre-wrap ${!val1 ? "text-slate-600 italic" : "text-slate-300"}`}
                    >
                      {val1 || "(Trống)"}
                    </div>
                  </div>
                  ...
                  >
                    <div
                      className={`text-sm line-clamp-4 whitespace-pre-wrap ${!val2 ? "text-slate-600 italic" : "text-slate-300"}`}
                    >
                      {val2 || "(Trống)"}
                    </div>
                  </div>
